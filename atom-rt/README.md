...
### Hub の 2 画面（同じファーム・同じ :8794）

用途で画面を分けています。ファームは 1 本（`rt-usb`）のままです。

| 画面 | パス | 主な用途 |
|---|---|---|
| **実機テレメトリ（M5）** | `/m5-telemetry` | 機体（右脚）。バー・足圧・カメラ・本記録。ライブ指令は 100〜170° |
| **ATOM 机上ラボ** | `/atom-bench` | 机上の 1 サーボ。校正、AS5600 なしの単体テスト。指令は 40〜230° |

どちらでもトポロジ・関節プロファイル・NVS を確認できます。机上ラボは Lab を保つ前提で、Robot のときは警告と「Lab に戻す」を出します。

机上ラボの画面構成は次の 2 点だけ実機テレメトリと違います。

- **トポロジは左に常設**。サーボ・校正・プロファイル・NVS のどのタブを開いても配線と磁石が見えるので、挿抜しながら作業できます（タブの「トポロジ」は廃止）
- **画面下端に固定ステータスバー**。ブリッジ / ATOM / モード / PWM / 指令角 / AS5600 生角と磁石 / V・A・W / 周期と実効 Hz / I2C エラー（累計と毎秒の増分）/ 8Servos を 20 Hz で更新します。I2C エラーが増えている間は赤く点滅するので、経路の設定漏れがすぐ分かります

机上ラボの指令は Socket.IO の `joint` / `out` に **`wide: true`** を付けて送ります。PC 側の狭いクランプ（`cop_ankle_ctrl.clamp_cmd` の 100〜170°）を通さず、`clamp_cal_deg` の 40〜230° になります。
...
20 Hz の電源監視は関節プロファイルの `ina_hub / ina_ch / ina_addr` に従い、**有効な関節枠**だけを見ます。トポロジで INA226 を選んで関節に割り当てるか、「関節 / 試験」の INA 一覧から選べます。`ina_addr=0` または関節が無効なら、その枠は I2C しません。既定は関節 0〜1 のみ PaHub 0x71 CHi。

欠測は文字列 `nan` ではなく `ok=0` + ダミー 0 です。失敗理由は数値（`UsbReason`）。Python の `REASON` と揃えています。

### 関節プロファイル

論理関節番号と、実際のエンコーダ / アクチュエータ / INA 経路の対応表です。机の配線と機体が違っても **再フラッシュせず** NVS に書けます。`lab_debug.py` の「関節プロファイル」タブ、トポロジの割当、または Hub のプロファイル画面から変えられます。

1 メッセージに全軸の `JointRoute`（10 バイト × 8）と `FootRoute`（3 バイト）、続けて有効マスク 2 バイト（`jen` / `fen`）を載せます。経路と有効は別なので、機体のアドレスを残したまま机上で未配線軸の I2C / PWM を止められます。
...
- 有効 … 既定は 8 軸も右足も ON。NVS `jprof/jen`（bit i）と `jprof/fen`
