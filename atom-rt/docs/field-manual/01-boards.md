...
- ボタン: GPIO41
- USB: ネイティブ USB（USB-Serial-JTAG）。ファームは `ARDUINO_USB_CDC_ON_BOOT=1` で仮想 COM
- 底面ピン: G5 / G6 / G7 / G8 / G38 / G39

`platformio.ini` のピン（SDA=2、SCL=1、LED=35、ボタン=41）はこの Lite に合わせています。Arduino の `Serial` は **HWCDC** です。TinyUSB の `Serial.enableReboot()` はありません。ダウンロードモード用のリセットは、レジスタ `RTC_CNTL_USB_RESET_DISABLE` で抑えています（DTR で書き込み中に落ちないように）。書き込みそのものは、従来どおり緑 LED のダウンロードモードが必要です。
...
| `0x9000` | **NVS**（約 20 KB） | キー・値。校正と関節経路と有効マスク |
...
3. **コードはフラッシュから直接実行する（XIP）。** NVS の読み書きのあいだ、ESP-IDF はフラッシュキャッシュを止めます。その瞬間、命令取り出しと USB の送出が同時に苦しくなります。詳しくは「層」の章。
...
| `jprof` | **`en`** | **正本。** 2 バイト blob。`[関節マスク, 足]` |
| `jprof` | `jen` | 旧・互換。関節 8 bit。新規ボードでは `en` を先に書く |
| `jprof` | `fen` | 旧・互換。右足 0/1 |
| `cal` | `mk0`…`mk7` | その軸のマップが有効か |
| `cal` | `mn0`… | 点数 |
| `cal` | `mx0` / `my0` … | unwrap 角 → 指令角の float 配列（1 本あたり約 764 B） |

再フラッシュせずに配線を変える、というのがプロファイルの存在理由です。**経路を消さずにポーリングだけ止める**のが有効マスクです。マップのキー名は実験リポジトリ `atoms3-as5600-test` の robot ファームと揃えています。

Hub の NVS タブに出る灰色の行は「ファームが知っている名前」で、フラッシュに無いと空です。空欄は故障ではなく、まだ焼いていないキーです。値が本当に読めているかは、ダンプ完了（`nvs.ok`）を見てください。
...
確認は `pio device monitor` ではなく `python tools/rt_usb_log.py`（バイナリなのでモニタは文字化けします）。HELLO の `ver=14` が出れば今のプロトコルです。
