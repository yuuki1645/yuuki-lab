...
いまの総合デバッグは **PC が PWM を握って掃引**し、できたマップを USB で NVS に送ります。40→230→40° を 1° 刻み。形式は `as5600-servo-map-v1`（`cal_map_chN.json`）。Hub の校正タブ（机上ラボ / 実機テレメトリ）からブラウザで保存・読込できます。`lab_debug.py` の Tk にも同じボタンが残っています。正本を PC の JSON に置くと、NVS 全文ダンプに頼らなくて済みます。読込は画面の対象 ch へ送り、ファイル内の channel は記録だけです。
