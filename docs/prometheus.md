...
動画・発表では **プロメテウス**、実装パッケージ名は `mujoco_rl_sim.dispatch` です。
...
ギリシャ神話の **プロメテウス** は、神々の火を人間に渡した存在です。  
このシステムでは比喩的に、

- **火** … 1 台では回しきれない計算量のある RL 学習  
- **人間の手元** … 自宅 LAN 上の PC 群  

として、「**学習任務を複数マシンに配る装置**」という位置づけです。
...
動画では省いてよい細部です。興味がある人向けの一行まとめです。
...
この Markdown をそのまま台本の骨格にできます。

| 章（ドキュメント） | 動画で見せる画面の例 |
|--------------------|----------------------|
| §1 なぜ作ったか | 1 台 PC で学習が長時間かかる様子 |
| §3 全体像 | 図1（3 役）を Canva やスライドに |
| §4 Pull | 図2（シーケンス）＋ Worker ログが動く録画 |
| §5 sweep | `baseline_10seed.yaml` と CLI register、ジョブ数表示 |
| §6 状態 | Web UI で queued → running → succeeded が増える |
| §8 ロボとの関係 | MuJoCo / 実機映像で締め |

**ナレーションのコツ**

- 初出は **司令室・部隊・任務カード** の日本語 → 字幕で Coordinator / Worker / Job  
- 「Pull」を **「空いている PC が次の仕事を取りに行く」** と言い換える  
- 最後に **「中身は自宅 LAN のジョブ配備。だから等身大ロボの試行回数を増やせる」** で締める  

### Mermaid のプレビュー

Cursor / VS Code では拡張 **Markdown Preview Mermaid Support** を入れ、`Ctrl+Shift+V` でプレビューできます。  
Canva 用には [Mermaid Live Editor](https://mermaid.live) から PNG/SVG を書き出して貼り付けてください。
...
*最終更新: 2026-06（プロメテウス初版・動画向けドキュメント）*
