...
| §2 全体像 | 図1（3 役）を Canva やスライドに |
| §3 Pull | 図2（シーケンス）＋ Worker ログが動く録画 |
| §4 sweep | `baseline_10seed.yaml` と CLI register、ジョブ数表示 |
| §5 状態 | Web UI で queued → running → succeeded が増える |
| §7 ロボとの関係 | MuJoCo / 実機映像で締め |
