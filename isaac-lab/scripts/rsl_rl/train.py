...
# WandB 同期モード（デフォルト: online）。
# グローバル設定（~/.config/wandb/settings が offline 等）より優先する。
...
def _scale_ppo_minibatches(agent_cfg: RslRlBaseRunnerCfg, num_envs: int) -> None:
    """並列 env 数に応じてミニバッチ数を調整し、1 ミニバッチ ≈2048 サンプルを目標にする。"""
    rollout = num_envs * agent_cfg.num_steps_per_env
    target = 2048
    agent_cfg.algorithm.num_mini_batches = max(8, rollout // target)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    _scale_ppo_minibatches(agent_cfg, env_cfg.scene.num_envs)
    print(
        f"[INFO] Training with num_envs={env_cfg.scene.num_envs}, "
        f"num_mini_batches={agent_cfg.algorithm.num_mini_batches}, "
        f"rollout_steps={env_cfg.scene.num_envs * agent_cfg.num_steps_per_env}"
    )
