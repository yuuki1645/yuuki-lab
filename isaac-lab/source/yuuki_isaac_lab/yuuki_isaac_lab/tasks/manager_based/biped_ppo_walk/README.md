...
| `BipedPpoWalkEnvCfg_PLAY.robot_visual_opacity` | `0.35`（`basket_thigh` かごだけ半透明） |

headless 実行時、IMU フレームは設定値に関わらず自動で無効化される。学習タスク（`-v0`）を GUI 付きで動かすときに表示したい場合は、`debug_vis_imu_frame` を `True` に変更する。透明度は `robot_visual_opacity`（0〜1）で **かご本体のみ** 調整でき、脚などには掛かからない。物理・観測には影響しない。
