...
class BipedPpoWalkEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # basket_thigh（かご本体）見た目の不透明度（1.0=不透明, 0.0=完全透明）。
    # IMU はかご内にあるため、ここだけ半透明にするとフレームが見える。
    # 脚など子ボディには適用しない。1.0 未満のときだけ有効。物理・観測には影響しない。
...
class BipedPpoWalkEnvCfg_PLAY(BipedPpoWalkEnvCfg):
    ...
    # Play 時は basket_thigh（かご）だけ半透明にして、内部の IMU フレームを見えるようにする。
