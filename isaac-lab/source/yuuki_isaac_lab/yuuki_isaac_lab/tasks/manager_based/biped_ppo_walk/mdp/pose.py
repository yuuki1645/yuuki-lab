...
"""Body-frame pose metrics derived from root orientation."""
...
def body_xaxis_world(quat_wxyz: torch.Tensor) -> torch.Tensor:
    """World-frame body +X axis from root quaternion [num_envs, 3]."""
...
def imu_zaxis_world(quat_wxyz: torch.Tensor) -> torch.Tensor:
    """World-frame up axis at the IMU site [num_envs, 3]."""
    w, x, y, z = quat_wxyz[:, 0], quat_wxyz[:, 1], quat_wxyz[:, 2], quat_wxyz[:, 3]
    zx = 2.0 * (x * z + w * y)
    zy = 2.0 * (y * z - w * x)
    zz = 1.0 - 2.0 * (x * x + y * y)
