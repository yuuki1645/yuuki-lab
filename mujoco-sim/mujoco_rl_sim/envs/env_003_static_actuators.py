...
"""002 と同 MJCF・観測だが、関節指令をリセット姿勢で固定し ``mj_step`` のみ進める環境。"""
...
class Env003StaticActuators(gym.Env):
    """
    ``Env002FullActuators`` と同じ MJCF・観測次元だが、**行動で関節目標を変えない**。

    - ``reset`` で決めた論理角初期姿勢に対応する ``data.ctrl`` を **エピソード中ずっと固定**し、
      各 ``step`` でそれを再適用したうえで ``mj_step`` のみ行う（指令による関節変化なし）。
    - 観測・終了条件・報酬の骨格は 002 に揃える（加速度が指令由来かを切り分けるためのベースライン）。
    """
    ...
    def step(self, action):
        ...
        nu = int(self.model.nu)
        delta_logical = np.zeros(nu, dtype=np.float32)
