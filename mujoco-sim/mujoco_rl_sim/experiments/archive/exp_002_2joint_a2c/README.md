...
チェックポイントは **`experiments/exp_002_2joint_a2c/checkpoints/run_YYYYMMDD_HHMMSS/`** 以下に保存される（**train のたびに新しい `run_*` ディレクトリ**）。`--resume` / `--checkpoint` の相対パスは **この実験フォルダ基準**（例: `checkpoints/run_.../final.pt`）。絶対パスも可。
