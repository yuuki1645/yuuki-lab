"""exp_005 の Gym 風環境ラッパー（exp_004 と同一: shaping / 姿勢終了 / 接触終了）。
...
class Env2JointPPO:
  """exp_005 用環境（model/main.xml）。観測・報酬・終了は exp_004 と同一。
  ...
  def reset(self):
    ...
    # policy_obs: 正規化済み観測（PolicyObs）→ ポリシー入力。step_physics は reset では不要
  ...
  def step(self, action, visualize: bool = False, episode_step: int = 0):
    ...
    #   policy_obs   … PolicyObs。clip/正規化済み（おおよそ [-1, 1]）。train のニューラルネット入力
