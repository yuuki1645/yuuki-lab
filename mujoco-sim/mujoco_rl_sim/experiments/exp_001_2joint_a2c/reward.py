...
そのため次を組み合わせる。
  ...
  3. 膝の形 … 人間らしい屈曲方向を促し、逆方向を減点
  4. 前傾・低姿勢ペナルティ … 倒れる直前の「前のめり」「しゃがみ」を早めに減点
  5. 転倒ペナルティ … env.py で終了ステップに一度だけ付与（本ファイル外）

1 ステップの合計（env 適用前）::

  total = forward + upright + knee_flex_bonus
          - knee_wrong_penalty - lean_penalty - height_penalty
...
class Reward:
  ...
  def compute(
    ...
    *,
    ...
    """
    # --- 膝: 逆屈曲のペナルティ -----------------------------------------------
    # knee_angle < 0 は「人間と逆」の屈曲。閾値 KNEE_WRONG_THRESH_RAD までは許容。
    # 理由: 歩行らしい関節使いを促し、膝を前に折るような動きを抑える。
