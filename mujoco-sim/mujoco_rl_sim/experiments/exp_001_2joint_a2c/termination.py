...
REASON_LOW_UPRIGHT = "low_upright"  # 体軸が鉛直から大きく外れた（横倒し・後傾など）
REASON_FORWARD_LEAN = "forward_lean"  # 前傾しすぎ（顔下がり）
...
class Termination:
  ...
  入力は observation.ObservationRaw と同じ物理量:
    ...
    - imu_zaxis_x : imu 体軸のワールド X 成分。負 = 前方向（+x）へ傾いている
  ...
  def done_reason(
    ...
    *,
    ...
    # (2) 直立度不足: 体軸が鉛直から外れすぎ → 横倒し・大きな後傾など
    if upright < config.MIN_IMU_UPRIGHT:  # 既定 0.55（1=完全直立）
      return REASON_LOW_UPRIGHT

    # (3) 前傾過多: 体軸の X 成分が -MAX より小さい（より負）→ 前に倒れすぎ
    if imu_zaxis_x < -config.MAX_FORWARD_LEAN:  # 既定 -0.40 未満で終了
