...
def _parse_args() -> TrainRunConfig:
  ...
  p.add_argument(
    ...
    type=str,
    default=None,
    help="再開する .pt（相対パスは exp_002 ディレクトリ基準）。新 checkpoint / wandb run を作成",
  )
  p.add_argument(
    "--lr",
    type=float,
...
def _create_agent(run: TrainRunConfig) -> tuple[AgentExp002A2C, dict[str, Any] | None]:
  ...
  agent = AgentExp002A2C.from_checkpoint(
    run.resume_path,
    lr=run.lr,
