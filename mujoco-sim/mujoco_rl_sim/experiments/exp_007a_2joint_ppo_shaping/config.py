"""exp_007a: exp_006 + 前進報酬の足底接地条件（ablation 007a）。

exp_006 と同一だが FORWARD_REQUIRE_FOOT_CONTACT=True のみ変更。
前進報酬（imu dx / foot dx）は foot_plate が床に接地している step のみ付与する。
"""
...
MAX_IMU_Z = 1.2
MIN_IMU_Z_NORM = 0.0
# 膝・IMU と heel_bottom_site の相対 XZ（大腿+下腿+足のおおよそのスパン）
...
WANDB_RUN_NAME = ""
WANDB_ENTITY = ""
WANDB_TAGS = ("ppo", "2joint", "shaping", "foot_geom_obs", "forward_require_foot_contact")
...
def training_config_dict() -> dict:
  ...
  return {
    ...
    "max_rel_heel_offset": MAX_REL_HEEL_OFFSET,
    "obs_dim": OBS_DIM,
