...
class RewardBreakdown:
  ...
  flight_low_upright_penalty: float
  hop_dx_band_bonus: float
  ...
  def shaping(self) -> float:
    ...
    return (
      ...
      - self.flight_low_upright_penalty
    )
...
class Reward:
  ...
  def _forward_imu_lean_multiplier(
    ...
    """飛翔中の前傾が強いほど IMU 前進報酬を減衰（exp_009）。強い前傾はゼロ（exp_011）。"""
    if not foot_on_floor and float(imu_zaxis_x) > config.FORWARD_IMU_MAX_LEAN_FOR_REWARD:
      return 0.0
  ...
  @staticmethod
  def _hop_dx_band_bonus(
    dx: float, *, upright: float, foot_on_floor: bool
  ) -> float:
    if config.HOP_DX_BAND_FLIGHT_ONLY and foot_on_floor:
      return 0.0
    if upright < config.HOP_DX_BAND_MIN_UPRIGHT:
      return 0.0
    if config.HOP_DX_BAND_MIN <= float(dx) <= config.HOP_DX_BAND_MAX:
      return config.HOP_DX_BAND_BONUS
    return 0.0

  @staticmethod
  def _survival_bonus(*, upright: float, imu_z: float) -> float:
    if upright < config.SURVIVAL_MIN_UPRIGHT:
      return 0.0
    if float(imu_z) < config.SURVIVAL_MIN_IMU_Z:
      return 0.0
    return config.SURVIVAL_BONUS_SCALE

  @staticmethod
  def _flight_low_upright_penalty(
    upright: float, *, foot_on_floor: bool
  ) -> float:
    if foot_on_floor:
      return 0.0
    deficit = max(0.0, config.FLIGHT_LOW_UPRIGHT_THRESH - float(upright))
    return deficit * config.FLIGHT_LOW_UPRIGHT_PENALTY_SCALE
  ...
  def _landing_bonus(step_physics: StepPhysics, *, hop: HopStepContext) -> float:
    ...
    if step_physics.imu_zaxis_x > config.LANDING_MAX_FORWARD_LEAN:
      return 0.0
    if step_physics.upright < config.LANDING_MIN_UPRIGHT:
      return 0.0
  ...
  def compute(
    ...
    *,
    ...
    return RewardBreakdown(
      ...
      flight_low_upright_penalty=self._flight_low_upright_penalty(
        upright, foot_on_floor=foot_on_floor
      ),
      hop_dx_band_bonus=self._hop_dx_band_bonus(
        step_physics.dx,
        upright=upright,
        foot_on_floor=foot_on_floor,
      ),
      survival_bonus=self._survival_bonus(
        upright=upright, imu_z=step_physics.imu_z
      ),
