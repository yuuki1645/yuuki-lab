"""exp_015: exp_010 + ホップ周期完了ボーナス（離地→着地で前進量を評価）。

exp_010 final が現状ベスト (~3.1 m)。マイルストーン/姿勢緩和は 3 m 未満で悪化。
...
# --- shaping: ホップ周期完了（離地 IMU x → 着地、exp_015）----------------------
HOP_CYCLE_BONUS = 4.0
HOP_CYCLE_MIN_FLIGHT_STEPS = 5  # 50 Hz で約 100 ms 以上の飛翔
HOP_CYCLE_MIN_DX_M = 0.06
HOP_CYCLE_MIN_UPRIGHT = 0.58
HOP_CYCLE_MAX_LANDING_LEAN = 0.26  # 着地時の imu_zaxis_x（前傾ダイブ抑制）
...
def training_config_dict() -> dict:
  ...
  return {
    ...
    "hop_cycle_min_dx_m": HOP_CYCLE_MIN_DX_M,
