...
class Reward:
  ...
  @staticmethod
  def _imu_fall_penalty(*, imu_dz: float, foot_on_floor: bool) -> float:
    """非接地中の急な IMU 高さ低下を抑制（前傾ダイブ墜落の手前）。"""
    if foot_on_floor or float(imu_dz) >= -config.IMU_FALL_PENALTY_DZ_THRESH:
      return 0.0
    excess = -float(imu_dz) - config.IMU_FALL_PENALTY_DZ_THRESH
    return excess * config.IMU_FALL_PENALTY_SCALE
  ...
  def compute(
    ...
    *,
    ...
    return RewardBreakdown(
      ...
      imu_fall_penalty=self._imu_fall_penalty(
        imu_dz=hop.imu_dz, foot_on_floor=foot_on_floor
      ),
