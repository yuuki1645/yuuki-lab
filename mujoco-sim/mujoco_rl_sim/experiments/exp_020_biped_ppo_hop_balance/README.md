# exp_020: 両脚バイペッド前進 PPO（contract 駆動）
...
|------|---------|---------|
| 学習本体 | 同一 | 同一 |
| テレメトリ payload | `telemetry/biped_ppo.py`（契約ラッパ） | `contract` 直結 |
| `train.py` | フル実装 | `run_ppo_train` に委譲 |
| 観測検証 | なし | `PolicyObs.to_vector` で契約 assert |
