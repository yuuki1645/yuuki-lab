# exp_021: 足裏 25 cm 両脚バイペッド PPO（contract 駆動）

## 概要

**exp_020 のコピー**で、**足裏アルミ板の +X 長さを 30 cm → 25 cm** に変更した実験です。  
学習・観測 42 次元・契約 `biped_ppo_v1`・ハイパラは exp_020 と同一です。

| 項目 | exp_020 | exp_021 |
|------|---------|---------|
| 足裏板 +X 長 | 0.30 m | **0.25 m** |
| 足裏板 mass（各脚） | 0.271 kg | 0.226 kg |
| その他 | — | exp_020 と同一 |
...
| 項目 | exp_019 | exp_021 |
|------|---------|---------|
| 学習本体 | 同一 | 同一（exp_020 経由） |
| テレメトリ | 契約ラッパ / contract 直結 | **contract 直結** |
| モデル | 足裏 30 cm | **足裏 25 cm** |
...
| **backward_lean_penalty** | `max(0, −imu_zaxis_x − LEAN_BACKWARD_THRESH) × LEAN_BACKWARD_PENALTY_SCALE` | `THRESH=0.12`, `SCALE=3.0` |
| **forward_lean_penalty** | 両足非接地かつ `aerial_steps ≥ LEAN_FORWARD_MIN_AERIAL_STEPS` のとき、`max(0, imu_zaxis_x − LEAN_FORWARD_THRESH) × SCALE` | `THRESH=0.14`, `MIN_AERIAL=2`, `SCALE=4.0` |
...
| `imu_zaxis_x < −0.38`（後傾） | `backward_lean` | **−30** |
...
```bash
cd mujoco-sim
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.train
```

高速学習のみ（ビューア・実時間待ちオフ）:

```bash
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.train --no-viewer --step-wall-sleep 0
```

ビューアを表示したまま最速（毎ステップ sync のみ・sleep なし）:

```bash
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.train --viewer-fast
# または
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.train --step-wall-sleep 0
```
...
```bash
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.train --viewer-fast --no-telemetry
```
...
```powershell
cd exp_021_biped_ppo_hop_balance
...
```bash
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.visualize
python -m mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.visualize --checkpoint runs/.../latest.pt
```
...
| 項目 | exp_020 | exp_021 |
|------|---------|---------|
| 学習・報酬・env コード | 基準 | **同一コピー** |
| 足裏アルミ板 +X | 0.30 m | **0.25 m** |
| 比較ベースライン | exp_019 | **exp_020** |
