"""exp_022: 両脚バイペッド前進 PPO 環境（contact 走査なし）。"""
...
class EnvBipedPPO:
  ...
  def step(self, action, visualize: bool = False, episode_step: int = 0):
    ...
    biped = self._episode.advance_biped_context(imu_z=imu_z)
    ...
    termination = self._termination.done_reason_pose(self.data)

    terminated = termination.terminated
    termination_reason = termination.reason

    reward = reward_result.total + termination.penalty
    ...
    step_info = {
      ...
      "in_stance": float(step_physics.in_stance),
      "flight_steps": float(biped.aerial_steps),
      "aerial_steps": float(biped.aerial_steps),
      "entered_stance": float(biped.entered_stance),
