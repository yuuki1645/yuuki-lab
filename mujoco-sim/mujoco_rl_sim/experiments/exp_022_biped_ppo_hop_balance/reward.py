"""両脚前進 PPO 向けステップ報酬（contact 走査なし）。"""
...
class Reward:
  ...
  @staticmethod
  def _aerial_duration_penalty(*, in_stance: bool, aerial_steps: int) -> float:
    if in_stance:
      return 0.0
  ...
  def _knee_hyperflex_penalty(
    ...
    *,
    ...
    if config.KNEE_HYPERFLEX_AERIAL_ONLY and in_stance:
      return 0.0
  ...
  def _forward_lean_penalty(
    imu_zaxis_x: float, *, in_stance: bool, aerial_steps: int
  ) -> float:
    if in_stance:
      return 0.0
  ...
  def compute(
    ...
    *,
    ...
    # 飛翔中の前傾が強いほど IMU 前進報酬を減衰（stance 中は 1.0）
    if not FORWARD_IMU_LEAN_GATE or in_stance:
    ...
    height_penalty = self._height_penalty(imu_z)
    flight_duration_penalty = self._aerial_duration_penalty(
      in_stance=in_stance, aerial_steps=biped.aerial_steps
    )
