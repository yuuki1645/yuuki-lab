# exp_023: 足裏 25 cm×10 cm・踵はみ出し 両脚バイペッド PPO（contract 駆動）

## 概要

**exp_021 のコピー**で、足裏アルミ板を次のとおり変更した実験です。

- **Y 幅 17 cm → 10 cm**（左右方向の板幅）
- **中心 X を -2.5 cm**（踵側が人間の踵より後方にはみ出す配置）

学習・観測 42 次元・契約 `biped_ppo_v1`・ハイパラは exp_021 と同一です。

| 項目 | exp_021 | exp_023 |
|------|---------|---------|
| 足裏板 +X 長 | 0.25 m | **0.25 m**（同一） |
| 足裏板 Y 幅 | 0.17 m | **0.10 m** |
| 足裏板中心 X（sole 基準） | 0.125 m | **0.100 m**（-2.5 cm） |
| 踵端 X（後端） | 0 m | **-0.025 m**（2.5 cm はみ出し） |
| その他 | — | exp_021 と同一 |
...
| 足裏アルミ板 Y 幅 | 0.17 m | **0.10 m** |
| 足裏板中心 X | 0.125 m | **0.100 m**（-2.5 cm） |
| 踵端 X | 0 m | **-0.025 m** |
| 比較ベースライン | exp_020 | **exp_021** |
