...
| 定数 | 既定値 | 意味 |
|------|--------|------|
| `MIN_IMU_Z` | **0.40 m** | **両足非接地**（どちらの `foot_plate` も床と非接触）のときの IMU 下限 |
| `MIN_IMU_Z_STANCE` | **0.34 m** | **どちらか足が床接地**のときの IMU 下限（しゃがみ歩行をやや許容） |
...
参考: 立位 keyframe の `imu_z ≈ 0.71 m`。転倒線 0.34〜0.40 m は歩行目標 IMU（0.50〜0.55 m）よりかなり低い。
