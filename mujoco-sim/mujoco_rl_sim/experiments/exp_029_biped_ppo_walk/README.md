...
| **比較は eval_compare** | 複数 run の `eval_report.json` を横並び。W&B / dispatch の eval 連携は未実装 |
...
```bash
python train.py --no-viewer --num-updates 300 \
...
```bash
python train.py --no-viewer
# または仮説の --set をそのまま継続
python train.py --no-viewer --set reward_enable_walk_shaping=true
```
...
各 job 完了後も **eval はローカル `eval_report.json`**。横断比較は `eval_compare`（dispatch の eval 台帳は未実装）。  
...
```bash
# 1) スモーク
python train.py --no-viewer --num-updates 300 --set forward_reward_scale=55

# 2) 比較
python scripts/eval_compare.py

# 3) 本番（Go の場合）
python train.py --no-viewer --set forward_reward_scale=55

# 4) 再比較
python scripts/eval_compare.py --csv compare.csv
```

### まだワークフローに含めないもの（優先 TODO）
