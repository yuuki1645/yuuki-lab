...
| ロールアウト | 単一 env 逐次 | **Subproc VecEnv**（`--num-envs`、推奨 8） |
| テスト / CI | なし | **pytest 34 本 + GitHub Actions** |
...
- 変更する係数・フラグをメモ（後で `config_effective.json` と照合）
- W&B を使う場合: `wandb login`（省略時は `--no-wandb` でローカルのみ）
...
```bash
python train.py --no-viewer --no-telemetry --num-envs 8 --num-updates 300 \
  --set forward_reward_scale=55.0
```

| 項目 | 推奨 |
|------|------|
| `--num-updates` | 200〜500（数分〜十数分） |
| `--no-viewer` | 常に付ける（スループット優先） |
| `--no-telemetry` | VecEnv 利用時は必須（`num_envs>1` では自動無効化） |
| `--num-envs` | 本番 PC では **8** 推奨（16C CPU 想定。詳細は下記 Subproc VecEnv） |
| `--set KEY=VAL` | 仮説の 1 変更をここで指定 |
| 終了後 | **自動 eval** → `runs/<run>/eval_report.json` |

学習のみ試したい場合: `python train.py --no-eval ...`（比較判断には eval が必要）

#### 2. 本番学習

スモークで Go なら本番。既定は `NUM_UPDATES=5000`・W&B 有効。

```bash
python train.py --no-viewer --no-telemetry --num-envs 8
# または仮説の --set をそのまま継続
python train.py --no-viewer --no-telemetry --num-envs 8 \
  --set reward_enable_walk_shaping=true
```

| オプション | 用途 |
|------------|------|
| `--num-envs N` | Subproc VecEnv の並列 env 数（`config.NUM_ENVS` 上書き） |
| `--resume <ckpt>` | 途中 ckpt から追加学習（**新 run ディレクトリ**・新 W&B run） |
| `--lr`, `--num-updates` | 再開時の上書き |
| `--no-wandb` | run 名が `run_YYYYMMDD_HHMMSS` になる |
| `--no-eval` | 学習後の自動 eval をスキップ（非推奨） |

**1 run の成果物**（`runs/exp_029_biped_ppo_walk/<run_name>/`）:

| ファイル | 内容 |
|----------|------|
| `config_effective.json` | その run で実際に効いた CLI・config スナップショット |
| `update_*.pt` / `latest.pt` | 途中・最新 ckpt |
| `final.pt` | 学習完了 ckpt（**eval の対象**） |
| `eval_report.json` | 公式採点（train 終了時に自動生成） |

run ディレクトリ名: W&B 有効時は Run Name（例: `lunar-pond-4`）、`--no-wandb` 時は `run_YYYYMMDD_HHMMSS`。
...
次の仮説へ進む前に、**何を変えたか**を `config_effective.json` とセットで残す。
...
| Subproc VecEnv（レベル2） | `sim/subproc_vec_env.py`、`--num-envs` |
...
| 4 | `NUM_ENVS` 本番化・N=12/16 ベンチ・sim hot path | ベンチ済（N=8）、既定はまだ 1 |
...
```powershell
python train.py --no-viewer --step-wall-sleep 0 --no-telemetry --num-envs 8
```

（`launch_parallel.ps1` はプロセス並列。1 プロセスあたりの VecEnv は別途 `--num-envs` で指定）
...
``NUM_ENVS > 1`` のとき、**子プロセスが MuJoCo ``step``、親プロセスが方策推論 + PPO 更新** を担当する（SB3 ``SubprocVecEnv`` 相当）。子に方策モデルは載せない。
...
| 項目 | 内容 |
|------|------|
| 既定 | ``NUM_ENVS=1``（`config.py`。本番 PC では **CLI で 8 推奨**） |
...
`config.CONTACT_SHANK_TERMINATES = False`（**exp_029 既定**）のとき:
