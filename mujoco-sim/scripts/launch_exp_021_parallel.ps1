...
#
# 使い方（mujoco-sim から）:
#   .\scripts\launch_exp_021_parallel.ps1
#   .\scripts\launch_exp_021_parallel.ps1 -Count 4
#
# 実行ポリシーでブロックされた場合:
#   powershell -ExecutionPolicy Bypass -File .\scripts\launch_exp_021_parallel.ps1
#
# ログをファイルに残す:
#   .\scripts\launch_exp_021_parallel.ps1 -LogDir logs\exp021
...
$TrainArgs = @(
  "-m", "mujoco_rl_sim.experiments.exp_021_biped_ppo_hop_balance.train",
  "--no-viewer",
  "--step-wall-sleep", "0",
  "--no-telemetry"
)
...
if ($RedirectLogs) {
  if ($LogDir -eq "") {
    $LogDir = Join-Path $MujocoSimRoot "logs\exp021_parallel"
  }
