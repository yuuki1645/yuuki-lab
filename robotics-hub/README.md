...
- **ATOM 机上ラボ** — 机上の ATOM + サーボ **1 本**で校正・単体テスト（`/atom-bench`）。同じ `tools/lab_debug.py`（:8794）につなぐのでファームの焼き分けは不要。Lab のまま **40〜230°**（ファーム可動域）を出し、AS5600 の **生角**・補正・ズレ、INA226 の V/A/W、トポロジ・プロファイル・NVS を確認できる
