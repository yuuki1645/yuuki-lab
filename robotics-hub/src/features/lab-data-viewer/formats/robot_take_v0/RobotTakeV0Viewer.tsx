...
export default function RobotTakeV0Viewer({ take, recorderBaseUrl }: LabFormatViewerProps) {
  ...
  const videoSrc = take.video_url ? recorderBaseUrl + take.video_url : null;
  ...
  return (
    <div className="lab-dv__format">
      <div className="lab-dv__format-meta">
        <span>format: <code>robot_take_v0</code></span>
        <span>
          video_t0_unix:{" "}
          {t0 != null ? t0.toFixed(3) : "—（meta に無し）"}
        </span>
        <span>
      ...
      <div className="lab-dv__format-grid">
        <section className="lab-dv__panel" aria-label="動画">
          ...
          {!videoSrc ? (
            <p className="lab-dv__hint">video.mp4 がありません。</p>
          ) : (
            <video
              ref={videoRef}
        ...
        <section className="lab-dv__panel lab-dv__panel--wide" aria-label="指令">
          <h3 className="lab-dv__panel-title">
            指令（±{CMD_WINDOW_SEC}s）
          </h3>
          {nearbyCmds.length === 0 ? (
            ...
            <ul className="lab-dv__cmd-list">
              {nearbyCmds.map((c, i) => (
                <li key={i}>
                  <code>{c.wall_unix.toFixed(3)}</code>{" "}
                  {c.endpoint ?? "?"}{" "}
